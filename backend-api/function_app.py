...
def VisitorCounter(req: func.HttpRequest) -> func.HttpResponse:
    ...
    # Ignore test traffic
    if visitor_id and ('cypress' in visitor_id.lower() or 'test' in visitor_id.lower()):
        logging.info(f"Ignoring test visitor: {visitor_id}")
        
        # Return a mock count with all expected fields
